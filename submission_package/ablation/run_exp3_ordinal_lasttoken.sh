#!/bin/bash
set -euo pipefail
export WANDB_MODE=offline CUDA_VISIBLE_DEVICES=2,3 PYTHONHASHSEED=0
cd /home/public/new_dl/aes2_gemma
ALL_FOLDS=data/processed/train_full.jsonl
OUT_ROOT=outputs/ablation/exp3_kfold
OOF_OUT=outputs/oof/exp3_lasttoken_oof.csv
for FOLD in 2 3 4; do
  echo "===== Exp3 Fold $FOLD ====="
  venv/bin/torchrun --nproc_per_node=2 stage2_train/sft_ordinal_v8.py     --config config/base_config_ordinal_v8.yaml     --fold $FOLD --all_folds $ALL_FOLDS     --output_dir $OUT_ROOT/fold_$FOLD     --num_train_epochs 2 --bf16 --flash_attention_2     --optim adamw_torch_fused     --per_device_train_batch_size 2 --gradient_accumulation_steps 8     --dataloader_num_workers 4     --no_attention_pooling
...
"
venv/bin/python optimize_thresholds_v8.py --oof_csv $OOF_OUT --output outputs/oof/exp3_thresholds.json
echo DONE
